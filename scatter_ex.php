...
if ($mode == "ext")
    ...
    $query = "select wafer_cnt, median from prober_data..bin_tool_match
    	where product='$dsid_req' and pid='$pid_req' and bin='$bin_req' and tester like '$tester_req%'
    ...
    while ($row = mssql_fetch_array($resultARef))
        {
        if ($first) { echo "[\"" . $row[0] . "\", " . $row[1] . "]"; $first = 0; }
